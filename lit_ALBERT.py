...
from transformers import AlbertForSequenceClassification, AdamW, AlbertTokenizer, BertConfig
from utils import macro_f1, weighted_f1
import dataloading as dl
import warnings

warnings.filterwarnings("ignore")
config = BertConfig('albert-large-v2', max_position_embeddings=128)


class LitALBERT(pl.LightningModule):

    def __init__(self):
        super(LitALBERT, self).__init__()

        self.model = AlbertForSequenceClassification.from_pretrained('albert-large-v2', num_labels=3)
        self.tokenizer = AlbertTokenizer.from_pretrained('albert-large-v2', config=config)
        self.train_data, self.val_data = random_split(dl.SemEvalDataset("datasets/preprocessed/albert_sciEntsBank_train.npy"),
                                                      [4472, 497], generator=torch.Generator().manual_seed(73))
        self.test_data = dl.SemEvalDataset("datasets/preprocessed/albert_sciEntsBank_test_ua.npy")
    ...
    def configure_optimizers(self):
        return AdamW(self.model.parameters(), lr=0.00001, correct_bias=False)
